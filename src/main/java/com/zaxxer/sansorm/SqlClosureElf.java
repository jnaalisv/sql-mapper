...
public final class SqlClosureElf {
    ...
    public static <T> int[] insertListBatched(Iterable iterable) {
        return SqlClosure.execute(c -> OrmElf.insertListBatched(c, iterable));
    }

    public static <T> int insertListNotBatched(Iterable iterable) {
        return SqlClosure.execute(c -> OrmElf.insertListNotBatched(c, iterable));
    }
    ...
    public static <T> List<T> executeQuery(Class<T> targetClass, final String sql, final Object... args) {
        return SqlClosure.execute(c -> OrmElf.resultSetToList(OrmElf.executeQuery(c, sql, args), targetClass));
    }
}
